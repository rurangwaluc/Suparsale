...
function getDateRange(
  ...
  const start = new Date(`${businessDate}T${fromTime || "00:00"}:00`);

  const end = toTime
    ? new Date(`${businessDate}T${toTime}:59`)
...
async function buildDailySummaryReport(input: {
  ...
  const report: DailySummaryReport = {
    businessName: "Suparsale Store Ltd",
...
export async function reportsRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (!query.success) {
        return reply.code(400).send({
          ok: false,
          message: "Invalid report date or time.",
        });
      }
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const pdfBuffer = await buildDailySummaryPdf(report);
      const timePart = query.data.fromTime || query.data.toTime
        ? `-${query.data.fromTime || "00-00"}-to-${query.data.toTime || "23-59"}`.replace(/:/g, "-")
        : "";
      const filename = `suparsale-daily-report-${businessDate}${timePart}.pdf`;
